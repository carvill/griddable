...
function Griddable<T>(props: GriddableProps<T>) {
    ...
    const onChange = (item: T): any => {
        ...
        const ids = selectedIds.length === 0 ? ([] as string[]) : selectedIds.join(",").split(",");
    ...
    const gridableBody = (props: GriddableProps<T>): any => {
        if (props.loading) {
        ...
        return (
            <Grid item xs={12}>
                {props.items.map((item: T, indexRow: number) => (
                    <GriddableRow key={indexRow} container onClick={props.onClicked ? handleClick(item) : undefined} className={props.onClicked ? 'GriddableRowClickable' : undefined}>
                        {props.columns.map((column: GriddableColumn<T>, indexColumn: number) => (
                            <GriddableCell key={indexColumn} column={column} item={item} index={indexColumn}
                                selectable={props.selectable && indexColumn === 0} mapper={props.mapper}
                                total={props.items.length} selected={selectedIds}
                                onChange={onChange} onChangeAll={onChangeAll} />
                        ))}
                    </GriddableRow>
                ))}
            </Grid>
        ...
        <Grid container>
            <Grid item xs={12}>
                <GriddableRowHeader container>
                    {props.columns.map((column: GriddableColumn<T>, index: number) => (
                        <GriddableCell key={index} column={column} index={index}
                            selectable={props.selectable && index === 0} mapper={props.mapper}
                            total={props.items.length} selected={selectedIds}
                            onChange={onChange} onChangeAll={onChangeAll} />
                    ))}
                </GriddableRowHeader>
